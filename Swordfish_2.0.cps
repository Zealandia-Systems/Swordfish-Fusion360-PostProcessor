...
legal = "Copyright (C) 2020-2024 by Zealandia Systems Ltd";
...
properties = {
	...
	/*useTCP: {
		title: "Use TCP programming",
		description: "The control supports Tool Center Point programming for multi-axis operations.",
		group: "multiAxis",
		scope: ["machine", "post"],
		type: "boolean",
		value: false
...
var gMotionModal = createOutputVariable({ control: CONTROL_FORCE, onchange: function () { if (skipBlocks) { forceModals(gMotionModal); } } }, gFormat); // modal group 1 // G0-G3, ...
var gPlaneModal = createOutputVariable({ control: CONTROL_FORCE, onchange: function () { if (skipBlocks) { forceModals(gPlaneModal); } forceModals(gMotionModal); } }, gFormat); // modal group 2 // G17-19
var gAbsIncModal = createOutputVariable({ control: CONTROL_FORCE, onchange: function () { if (skipBlocks) { forceModals(gAbsIncModal); } } }, gFormat); // modal group 3 // G90-91
...
function formatFeedMode(mode) {
	var fMode = (mode == FEED_PER_REVOLUTION) ? 95 : 94;
	if (fMode) {
...
function onSection() {
	...
	if (currentSection.type == TYPE_MILLING) {
		...
		writeComment(getParameter("operation-comment", "") + " - Milling - Tool: " + tool.number + " - " + tool.comment + " " + getToolTypeName(tool.type));
	}
	...
	if (currentSection.type == TYPE_JET) {
		...
		writeComment(getParameter("operation-comment", "") + " - Laser/Plasma - Cutting mode: " + getParameter("operation:cuttingMode"));
	}
	...
	if (insertToolCall || newWorkOffset || newWorkPlane || state.tcpIsActive) {
		...
		if (!isFirstSection() && (insertToolCall || state.tcpIsActive)) {
	...
	writeBlock(gPlaneModal.format(17), gAbsIncModal.format(90), feedMode);
...
function onDwell(seconds) {
	...
	var feedMode = gFeedModeModal.getCurrent();
	writeBlock(gFeedModeModal.format(94), gFormat.format(4), "P" + secFormat.format(seconds));
	writeBlock(gFeedModeModal.format(feedMode));
}
...
function onCyclePoint(x, y, z) {
	if (isInspectionOperation()) {
		...
		if (typeof writeProbeCycle == "function") {
		...
		switch (cycleType) {
			...
			case "counter-boring":
				writeBlock(
					...
					conditional(P > 0, secFormat.format(P)),
					feedOutput.format(F)
				);
				break;
			case "chip-breaking":
...
function writeInitialPositioning(position, isRequired, codes1, codes2) {
	...
	var hOffset = getSetting("outputToolLengthOffset", false) ? hFormat.format(tool.lengthOffset) : "";
